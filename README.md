...
- **Tích hợp Google Drive API v3 & Lưu ý Phân quyền Thư mục Hồ sơ Xe**: Phân hệ Quản lý Xe tích hợp hàm `searchVehicleDriveFile` (`src/services/googleDrive.ts`) quét trực tiếp thư mục `Hồ sơ Xe` (ID: `1UZ5ZUTPrOZ4-ClAbxCgTQ8pbn8d6CzSa`) qua Google Drive API v3 theo **Số khung (VIN)** (tuyệt đối không quét theo biển số xe). Hỗ trợ nhận diện cả **Tệp tin (File)** (PDF, RAR, ZIP, DOC...) lẫn **Thư mục (Folder)** mang tên Số khung xe (hoặc khớp 6 số cuối số khung) và tự động sinh đường dẫn xem phù hợp. Thư mục trên Google Drive bắt buộc phải được thiết lập Quyền truy cập chung là **"Bất kỳ ai có đường liên kết"** với vai trò **"Người xem"** để Google Drive API cho phép truy xuất không cần xác thực tài khoản Google từ trình duyệt.